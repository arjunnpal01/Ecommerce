...
import img1 from '/image/dried fruits.jpg';
import img2 from '/image/almonds.jpg';
import img3 from '/image/mango juice.jpg';
import img4 from '/image/dates.jpg';
import img5 from '/image/masala magics.jpg';
  ...
  return (
    <section className="py-10 px-6 md:px-20 bg-white">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-700">Day Of <span className='text-green-800'>The Deal</span></h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-5">
        {products.map((product, i) => (
          <div key={i} className="border border-gray-200 rounded-lg p-4 relative hover:shadow-md transition-all">
            ...
            {product.badge && (
              <span
                className={`absolute top-2 right-2 text-xs font-semibold px-2 py-1 rounded ${
                  product.badge === 'SALE' ? 'bg-green-800 text-white' : 'bg-gray-700 text-white'
                }`}
              >
                {product.badge}
              </span>
           ...
           <img src={product.image} alt={product.title} className="max-h-full object-contain mb-4 hover:" />
            </div>
